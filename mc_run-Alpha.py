...
def generate_data_compute_bc(alpha, amplitude, seed, dim, delay, skip, normalize = True, weighted = True):
    ...
    return np.array([1-levy_stable.fit(np.abs(S))[0] for S in Pxx])
...
plt.title("{} Alpha selector Test power estimated via {} MC iterations".format(metric, mc_iterations))
plt.savefig("NEW-alpha-amplitudes-magnitude-alpha-GoF-test-powers{}.pdf".format(mc_iterations))
plt.show()

np.savetxt("{}iterations_{}-metric_magnitude-alpha-test_powers_alpha1.1-2_amplitude0-2.txt".format(mc_iterations,metric),np.array(test_powers_alpha))
