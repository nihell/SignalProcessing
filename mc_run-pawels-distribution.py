...
def generate_data_compute_bc(alpha, amplitude, seed, dim, delay, skip, normalize = True, weighted = True):
    signal = create_signal(alpha, amplitude, seed)
    latest_visit = np.zeros((len(grid),2)) # for each grid index, first entry stores current measure, second entry stores index of latest visit
    for i in range(0,len(signal)-1):
        if signal[i] <signal[i+1]:
            for j in range(0,len(grid)):
                if grid[j] > signal[i] and grid[j]<= signal[i+1]:
                    latest_visit[j,1] = i
        elif signal[i] > signal[i+1]:
            for j in range(0,len(grid)):
                if grid[j]<= signal[i]:
                    latest_visit[j,0] = latest_visit[j,0] + (i-latest_visit[j][1])
...
plt.title("{} Distribution KS Test power estimated via {} MC iterations".format(metric, mc_iterations))
plt.savefig("distribution-ks-GoF-test-powers{}.pdf".format(mc_iterations))
plt.show()

np.savetxt("{}iterations_{}-metric_distribution-ks_test_powers_alpha1.6-2_amplitude5-9.txt".format(mc_iterations,metric),np.array(test_powers_alpha))
